  ...
  return (
    <AuthLayout title="Anmäl ditt barn" subtitle="Skapa ett konto för din familj">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <Input label="Telefonnummer" type="tel" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="070-000 00 00" required />
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="flex-1">
            <Input label="Gatuadress" value={form.street} onChange={e => setForm(f => ({ ...f, street: e.target.value }))} placeholder="Storgatan" required />
          </div>
          <div className="sm:w-24">
            <Input label="Nr" value={form.streetNumber} onChange={e => setForm(f => ({ ...f, streetNumber: e.target.value }))} placeholder="12" required />
          </div>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="sm:w-32">
            <Input label="Postnummer" value={form.postalCode} onChange={e => setForm(f => ({ ...f, postalCode: e.target.value }))} placeholder="123 45" required />
          </div>
          <div className="flex-1">
            <Input label="Postort" value={form.city} onChange={e => setForm(f => ({ ...f, city: e.target.value }))} placeholder="Stockholm" required />
          </div>
        </div>
